...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_direction_index = random.randint(0, 4)
    ...
    def init_food(self):
        ...
        available_food_positions = self.available_food_positions.difference(set(snake.blocks))
        if not self.custom:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if set(self.available_food_positions).intersection(set(self.snake.blocks)).__len__()!=4:
                self.snake.alive = False
            # Check if snake eats itself
            elif len(self.snake.blocks)!=len(set(self.snake.blocks)):
                self.snake.alive = False
            #  Check if snake eats the food
            if(self.FOOD in self.snake.blocks):
                # Remove old food
                self.world[self.FOOD[0], self.FOOD[1]] = 0
